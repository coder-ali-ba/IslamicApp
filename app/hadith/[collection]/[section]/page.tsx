...
export default async function HadithSectionPage({
  ...
  const arabicMap = new Map(
    arabicHadiths.map((hadith) => [String(hadith.hadithnumber), hadith.text]),
  );
  ...
  const urduMap = new Map(
    urduHadiths.map((hadith) => [String(hadith.hadithnumber), hadith.text]),
  );
  ...
  const hadiths: CombinedHadith[] = englishHadiths.map((hadith) => {
    ...
    return {
      ...
      english: hadith.text,
      arabic: arabicMap.get(String(hadith.hadithnumber)) ?? "",
      urdu: urduMap.get(String(hadith.hadithnumber)) ?? "",
      grades: uniqueGrades,
    };
  });
      ...
      <section className="relative overflow-hidden border-b border-stone-800 bg-stone-950 text-white">
        ...
        <div className="relative mx-auto max-w-7xl px-5 py-14 sm:px-6 md:py-16 lg:px-8 lg:py-20">
          ...
          <div className="mb-7 flex flex-wrap items-center gap-2 text-xs sm:text-sm">
            ...
            {/* <span className="text-stone-400">{collectionName}</span> */}
            <Link
              href={`/hadith/${collection}`}
              className=" text-sm cursor-pointer text-stone-400 transition hover:text-[#d6b56d]"
            >
            ...
            <span className="font-medium text-stone-300">{bookName}</span>
          </div>
          ...
          <p className="mt-4 max-w-2xl text-sm leading-7 text-stone-400 sm:text-base">
            Explore the narrations and teachings contained in this section of
